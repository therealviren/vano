...
static struct termios global_orig_termios;
static bool global_termios_saved = false;

void enableRawMode(TerminalState& state) {
    if (tcgetattr(STDIN_FILENO, &state.orig_termios) == -1) {
        die("tcgetattr");
    }
    global_orig_termios = state.orig_termios;
    global_termios_saved = true;
