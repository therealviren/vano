...
void Screen::scroll(Cursor& cursor, const Buffer& buffer) {
    ...
    if (cursor.cy >= static_cast<int>(buffer.lines.size())) {
        cursor.cy = buffer.lines.empty() ? 0 : static_cast<int>(buffer.lines.size()) - 1;
    }
    ...
    if (cursor.cy >= 0 && cursor.cy < static_cast<int>(buffer.lines.size())) {
        ...
        if (cursor.cx > static_cast<int>(line.size())) cursor.cx = static_cast<int>(line.size());
    ...
    if (cursor.cy < static_cast<int>(cursor.rowoff)) {
        cursor.rowoff = static_cast<size_t>(cursor.cy);
    }
    if (cursor.cy >= static_cast<int>(cursor.rowoff) + screen_rows) {
        cursor.rowoff = static_cast<size_t>(cursor.cy - screen_rows + 1);
    }
    if (visual_cx < static_cast<int>(cursor.coloff)) {
        cursor.coloff = static_cast<size_t>(visual_cx);
    }
    if (visual_cx >= static_cast<int>(cursor.coloff) + usable_cols) {
        cursor.coloff = static_cast<size_t>(visual_cx - usable_cols + 1);
    }
}
...
void Screen::checkBracketMatch(const Buffer& buffer, const Cursor& cursor, int& match_x, int& match_y) {
    ...
    if (cursor.cx < 0 || cursor.cx >= static_cast<int>(buffer.lines[static_cast<size_t>(cursor.cy)].size())) return;

    char current = buffer.lines[static_cast<size_t>(cursor.cy)][static_cast<size_t>(cursor.cx)];
    int dir = 0;
    char target = 0;
    ...
    while (y >= 0 && y < static_cast<int>(buffer.lines.size())) {
        if (x < 0 || x >= static_cast<int>(buffer.lines[static_cast<size_t>(y)].size())) {
            y += dir;
            if (y >= 0 && y < static_cast<int>(buffer.lines.size())) {
                x = (dir == 1) ? 0 : static_cast<int>(buffer.lines[static_cast<size_t>(y)].size()) - 1;
            }
            in_string = false;
            in_char = false;
            continue;
        ...
        char c = buffer.lines[static_cast<size_t>(y)][static_cast<size_t>(x)];

        if (c == '"' && (x == 0 || buffer.lines[static_cast<size_t>(y)][static_cast<size_t>(x - 1)] != '\\') && !in_char) in_string = !in_string;
        if (c == '\'' && (x == 0 || buffer.lines[static_cast<size_t>(y)][static_cast<size_t>(x - 1)] != '\\') && !in_string) in_char = !in_char;
...
void Screen::drawRows(const Buffer& buffer, const Cursor& cursor, const std::string& filename, EditorMode mode, int sel_sx, int sel_sy, int sel_ex, int sel_ey) {
    ...
    } else if (ext == ".sh" || ext == ".bash" || filename == "Makefile") {
    ...
    for (int y = 0; y < screen_rows; y++) {
        int filerow = y + static_cast<int>(cursor.rowoff);
        if (show_gutter) {
            output_buffer += theme.gutter_bg + theme.gutter_fg;
            if (filerow < static_cast<int>(buffer.lines.size())) {
                std::string gnum = std::to_string(filerow + 1);
                int padding = (gutter - 1) - static_cast<int>(gnum.size());
        ...
        if (filerow >= static_cast<int>(buffer.lines.size())) {
            if (buffer.lines.size() == 1 && buffer.lines[0].empty() && y == screen_rows / 3) {
                ...
                output_buffer += welcome.substr(0, static_cast<size_t>(welcomelen));
            } else {
                output_buffer += "~";
            }
        } else {
            int line_size = static_cast<int>(buffer.lines[static_cast<size_t>(filerow)].size());
            std::vector<std::string> syntax_colors(static_cast<size_t>(line_size), "");
            std::string current_word = "";
            std::vector<int> current_word_indices;
            bool in_string = false;
            char string_char = 0;
            for (int i = 0; i < line_size; ++i) {
                char c = buffer.lines[static_cast<size_t>(filerow)][static_cast<size_t>(i)];
                if (!in_string && (c == '"' || c == '\'' || c == '`')) {
                    in_string = true;
                    string_char = c;
                    syntax_colors[static_cast<size_t>(i)] = theme.string;
                    continue;
                }
                if (in_string && c == string_char && (i == 0 || buffer.lines[static_cast<size_t>(filerow)][static_cast<size_t>(i - 1)] != '\\')) {
                    in_string = false;
                    syntax_colors[static_cast<size_t>(i)] = theme.string;
                    continue;
                }
                ...
                if (c == '#' && (ext == ".py" || ext == ".sh" || ext == ".bash" || ext == ".lua" || filename == "Makefile")) {
                    for (int j = i; j < line_size; j++) {
                        syntax_colors[static_cast<size_t>(j)] = theme.comment;
                    }
                    break;
                }
                if (c == '/' && i + 1 < line_size && buffer.lines[static_cast<size_t>(filerow)][static_cast<size_t>(i + 1)] == '/' && (ext == ".cpp" || ext == ".hpp" || ext == ".h" || ext == ".cc" || ext == ".c" || ext == ".js" || ext == ".ts" || ext == ".jsx" || ext == ".tsx")) {
                    for (int j = i; j < line_size; j++) {
                        syntax_colors[static_cast<size_t>(j)] = theme.comment;
                    }
                    break;
                ...
                if (std::string("+-*/=<>!&|%^~?").find(c) != std::string::npos) {
                    syntax_colors[static_cast<size_t>(i)] = theme.operator_color;
                    continue;
                }
                ...
                if (std::isalnum(static_cast<unsigned char>(c)) || c == '_') {
                    ...
                    if (!current_word.empty()) {
                        ...
                        std::string color = "";
                        if (is_keyword) color = theme.keyword;
                        else if (is_type) color = theme.type;
                        else if (is_number) color = theme.number;
                        if (!color.empty()) {
                            for (int idx : current_word_indices) {
                                syntax_colors[static_cast<size_t>(idx)] = color;
                            }
                        }
                        current_word.clear();
            ...
            int current_vis_x = 0;
            for (size_t i = 0; i < buffer.lines[static_cast<size_t>(filerow)].size();) {
                if (buffer.lines[static_cast<size_t>(filerow)][i] == '\t') {
                    ...
                    size_t start_idx = i;
                    uint32_t cp = decodeUTF8(buffer.lines[static_cast<size_t>(filerow)], i);
                    std::string utf8_char = buffer.lines[static_cast<size_t>(filerow)].substr(start_idx, i - start_idx);
            ...
            if (cells.empty() && mode == MODE_VISUAL) {
                ...
                else if (filerow == sel_sy && filerow == sel_ey) is_selected = (0 >= sel_sx && 0 <= sel_ex);
                else if (filerow == sel_sy) is_selected = (0 >= sel_sx);
                else if (filerow == sel_ey) is_selected = (0 <= sel_ex);
                if (is_selected) {
                    output_buffer += theme.selection + " " + theme.reset;
                }
            }

            for (int c = static_cast<int>(cursor.coloff); c < static_cast<int>(cursor.coloff) + usable_cols; ++c) {
                if (c < static_cast<int>(cells.size())) {
                    ...
                    bool is_bracket = (filerow == by && real_idx == bx) ||
                                      (filerow == cursor.cy && real_idx == cursor.cx && bx != -1);
                    std::string syntax_color = cell.is_tab_pad ? "" : syntax_colors[static_cast<size_t>(real_idx)];

                    if (is_selected) output_buffer += theme.selection;
                    if (is_bracket) output_buffer += theme.bracket;
                    if (!is_bracket && !syntax_color.empty()) output_buffer += syntax_color;

                    output_buffer += cell.bytes;

                    if (!is_bracket && !syntax_color.empty()) output_buffer += theme.reset;
                    if (is_bracket) output_buffer += theme.reset;
                    if (is_selected) output_buffer += theme.reset;
                } else {
                    break;
                }
            }
...
void Screen::drawStatusBar(const Buffer& buffer, const Cursor& cursor, const std::string& filename, EditorMode mode) {
    if (mode == MODE_NORMAL) output_buffer += theme.status_normal;
    else if (mode == MODE_VISUAL) output_buffer += theme.status_visual;
    else if (mode == MODE_SEARCH) output_buffer += theme.status_search;
    else if (mode == MODE_COMMAND) output_buffer += theme.status_command;
    ...
    output_buffer += status.substr(0, static_cast<size_t>(len));
...
void Screen::drawMessageBar(const std::string& status_msg) {
    output_buffer += "\x1b[K";
    int len = static_cast<int>(status_msg.size());
    if (len > screen_cols) len = screen_cols;
    if (len > 0) output_buffer += status_msg.substr(0, static_cast<size_t>(len));
}
...
void Screen::refresh(const Buffer& buffer, Cursor& cursor, const std::string& filename, const std::string& status_msg, EditorMode mode, int sel_sx, int sel_sy, int sel_ex, int sel_ey) {
    ...
    int cursor_y = (cursor.cy - static_cast<int>(cursor.rowoff)) + 1;
    int cursor_x = (visual_cx - static_cast<int>(cursor.coloff)) + 1 + gutter;
