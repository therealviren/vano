...
void Screen::scroll(const Cursor& cursor, const Buffer& buffer) {
    const_cast<Cursor&>(cursor).rx = 0;
    int gutter = getGutterWidth(buffer.lines.size());
    int usable_cols = screen_cols - gutter;

    if (cursor.cy < static_cast<int>(cursor.rowoff)) {
        const_cast<Cursor&>(cursor).rowoff = cursor.cy;
    }
    if (cursor.cy >= static_cast<int>(cursor.rowoff) + screen_rows) {
        const_cast<Cursor&>(cursor).rowoff = cursor.cy - screen_rows + 1;
    }
    if (cursor.cx < static_cast<int>(cursor.coloff)) {
        const_cast<Cursor&>(cursor).coloff = cursor.cx;
    }
    if (cursor.cx >= static_cast<int>(cursor.coloff) + usable_cols) {
        const_cast<Cursor&>(cursor).coloff = cursor.cx - usable_cols + 1;
    }
}
